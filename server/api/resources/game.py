...
from server.api.resources.utils import datetime_type, get_model, get_user, get_team, ModelId, user_type
...
class GameResource(Resource):
    ...
    put_pars.add_argument('league_id', type=ModelId(League),
                          help="Неправильно указана лига")

    put_pars.replace_argument('team1_id', type=ModelId(Team),
                              help="Неправильный указана команда", dest='team1')
    put_pars.replace_argument('team2_id', type=ModelId(Team),
...
class GamesResource(Resource):
    ...
    def get(self):
        ...
        res = {
            'success': True,
            'games': [g.to_dict() for g in league.games]
        }
        return jsonify(res)
    ...
    post_pars.add_argument('league_id', type=ModelId(League), dest='league',
                           help="Неправильно указана лига", required=True)
    post_pars.replace_argument('team1_id', type=ModelId(Team), dest='team1',
                               required=True, help="Неправильный указана команда")
    post_pars.replace_argument('team2_id', type=ModelId(Team), dest='team2',
